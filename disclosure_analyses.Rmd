...
# mean-center network metrics of interest
fall_neg_dd_trait_data = fall_neg_dd_trait_data %>% 
  mutate_at(vars(Responsive_Total, Empathic_Total), 
            funs(ctr = mean_center(.)))

# does empathy of network predict disclosure? [NO]
empathic_net_got_help_model = glmer(GotHelp ~ Empathic_Total_ctr + (1 | ID/Dorm/Day), data=fall_neg_dd_trait_data, family=binomial, control=glmerControl(optimizer='bobyqa'))

# does responsiveness of network predict disclosure? [NO]
responsive_net_got_help_model = glmer(GotHelp ~ Responsive_Total_ctr + (1 | ID/Dorm/Day), data=fall_neg_dd_trait_data, family=binomial, control=glmerControl(optimizer='bobyqa'))
...
utility_lifsat_model = lmer(LifSat_day_avg_pz_next_day ~ LifSat_day_avg_pz + GotHelp + EmoRating_day_mag_ctr_next_day + (1 | ID) + (0 + LifSat_day_avg_pz|ID) + (0 + GotHelp|ID) + (0 + EmoRating_day_mag_ctr_next_day|ID), data = fall_neg_dd_trait_data_utility, na.action=na.exclude)

# does disclosure predict next day loneliness? [NO]
utility_lonely_model = lmer(Lonely_day_avg_pz_next_day ~ Lonely_day_avg_pz + GotHelp + EmoRating_day_mag_ctr_next_day + (1 | ID) + (0 + Lonely_day_avg_pz|ID) + (0 + GotHelp|ID) + (0 + EmoRating_day_mag_ctr_next_day|ID), data = fall_neg_dd_trait_data_utility, na.action=na.exclude)

# does disclosure predict next day positive affect? [NO]
utility_posaff_model = lmer(PosAff_day_avg_pz_next_day ~ PosAff_day_avg_pz + GotHelp + EmoRating_day_mag_ctr_next_day + (1 | ID) + (0 + PosAff_day_avg_pz|ID) + (0 + GotHelp|ID) + (0 + EmoRating_day_mag_ctr_next_day|ID), data = fall_neg_dd_trait_data_utility, na.action=na.exclude)

# does disclosure predict next day negative affect? [NO]
utility_negaff_model = lmer(NegAff_day_avg_pz_next_day ~ NegAff_day_avg_pz + GotHelp + EmoRating_day_mag_ctr_next_day + (1 | ID) + (0 + NegAff_day_avg_pz|ID) + (0 + GotHelp|ID) + (0 + EmoRating_day_mag_ctr_next_day|ID), data = fall_neg_dd_trait_data_utility, na.action=na.exclude)

# does disclosure predict next day stress? [NO]
utility_stress_model = lmer(Stress_day_avg_pz_next_day ~ Stress_day_avg_pz + GotHelp + EmoRating_day_mag_ctr_next_day + (1 | ID) + (0 + Stress_day_avg_pz|ID) + (0 + GotHelp|ID) + (0 + EmoRating_day_mag_ctr_next_day|ID), data = fall_neg_dd_trait_data_utility, na.action=na.exclude)
