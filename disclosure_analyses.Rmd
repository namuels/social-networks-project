...
## How do students' social relationships influence their likelihood of disclosure?
```{r, echo=FALSE, warning=FALSE}
...
# does empathy of network predict disclosure? [YES]
empathic_net_got_help_model = fall_neg_dd_trait_data %>% 
  group_by(ID) %>% 
  lm(disclosure_ratio_agg_z ~ Empathic_Total_z, data=.)
...
# does responsiveness of network predict disclosure? [YES]
responsive_net_got_help_model = fall_neg_dd_trait_data %>% 
  group_by(ID) %>% 
  lm(disclosure_ratio_agg_z ~ Responsive_Total_z, data=.)
...
Attributes of students' social networks were used to predict their likelihood of disclosure. Neither the number of empathic friends nor the number responsive friends a student reported having in their dorm predicted significant differences in likelihood of disclosure.
...
fall_neg_dd_trait_data_utility_sd = fall_neg_dd_trait_data %>% 
  ...
  select(ID, disclosure_ratio_pz, EmoRating_day_mag_ctr, 
         LifSat_day_avg_pz, Lonely_day_avg_pz, PosAff_day_avg_pz, NegAff_day_avg_pz, Stress_day_avg_pz)
...
# does disclosure predict same-day loneliness? [YES]
...
# does disclosure predict same-day negative affect? [YES]
...
# does disclosure predict same-day stress? [YES]
...
fall_neg_dd_trait_data_utility_nd = fall_neg_dd_trait_data %>% 
  ...
  mutate(EmoRating_day_mag_ctr = mean_center(EmoRating_day_mag)) %>% 
  ungroup() %>% 
  # creating time-lagged daily stress magnitude column
  group_by(ID) %>% 
  mutate(EmoRating_day_mag_ctr_next_day = lead(EmoRating_day_mag_ctr, order_by = ID)) %>% 
  ungroup() %>% 
  # select relevant columns for utility analyses
  select(ID, GotHelp, EmoRating_day_mag_ctr_next_day, 
         LifSat_day_avg_pz, Lonely_day_avg_pz, PosAff_day_avg_pz, NegAff_day_avg_pz, Stress_day_avg_pz, 
         LifSat_day_avg_pz_next_day, Lonely_day_avg_pz_next_day, PosAff_day_avg_pz_next_day, NegAff_day_avg_pz_next_day, Stress_day_avg_pz_next_day)

# does disclosure predict next day satisfaction with day? [NO]
utility_lifsat_model_nd = lmer(LifSat_day_avg_pz_next_day ~ LifSat_day_avg_pz + GotHelp + EmoRating_day_mag_ctr_next_day + (1 | ID) + (0 + LifSat_day_avg_pz|ID) + (0 + GotHelp|ID) + (0 + EmoRating_day_mag_ctr_next_day|ID), data = fall_neg_dd_trait_data_utility_nd, na.action=na.exclude)

# does disclosure predict next day loneliness? [NO]
utility_lonely_model_nd = lmer(Lonely_day_avg_pz_next_day ~ Lonely_day_avg_pz + GotHelp + EmoRating_day_mag_ctr_next_day + (1 | ID) + (0 + Lonely_day_avg_pz|ID) + (0 + GotHelp|ID) + (0 + EmoRating_day_mag_ctr_next_day|ID), data = fall_neg_dd_trait_data_utility_nd, na.action=na.exclude)

# does disclosure predict next day positive affect? [NO]
utility_posaff_model_nd = lmer(PosAff_day_avg_pz_next_day ~ PosAff_day_avg_pz + GotHelp + EmoRating_day_mag_ctr_next_day + (1 | ID) + (0 + PosAff_day_avg_pz|ID) + (0 + GotHelp|ID) + (0 + EmoRating_day_mag_ctr_next_day|ID), data = fall_neg_dd_trait_data_utility_nd, na.action=na.exclude)

# does disclosure predict next day negative affect? [NO]
utility_negaff_model_nd = lmer(NegAff_day_avg_pz_next_day ~ NegAff_day_avg_pz + GotHelp + EmoRating_day_mag_ctr_next_day + (1 | ID) + (0 + NegAff_day_avg_pz|ID) + (0 + GotHelp|ID) + (0 + EmoRating_day_mag_ctr_next_day|ID), data = fall_neg_dd_trait_data_utility_nd, na.action=na.exclude)

# does disclosure predict next day stress? [NO]
utility_stress_model_nd = lmer(Stress_day_avg_pz_next_day ~ Stress_day_avg_pz + GotHelp + EmoRating_day_mag_ctr_next_day + (1 | ID) + (0 + Stress_day_avg_pz|ID) + (0 + GotHelp|ID) + (0 + EmoRating_day_mag_ctr_next_day|ID), data = fall_neg_dd_trait_data_utility_nd, na.action=na.exclude)

```

Students' daily disclosure activity was used to predict satisfaction, loneliness, positive and negative affect, and stress the following day. Disclosing stress to a dorm-mate did not predict significant changes in these measures of well-being the following day.
